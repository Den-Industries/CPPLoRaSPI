...
LoRaClass::LoRaClass()
{
    ...
    _packetIndex = 0;
    spi_config.mode=0;
      spi_config.speed=200000;
      spi_config.delay=0;
      spi_config.bits_per_word=8;

      mySPI=new SPI("/dev/spidev4.1",&spi_config);
...
int LoRaClass::begin(long frequency)
{
  ...
  // start SPI
  if(mySPI->begin())
    std::cout << "SPI INITED SUCCESFULLY" << std::endl;
...
int LoRaClass::write(uint8_t byte)
{
  return write(&byte, sizeof(byte));
}

int LoRaClass::write(const uint8_t *buffer, int size_v)
{
  int currentLength = readRegister(REG_PAYLOAD_LENGTH);

  // check size
  if ((currentLength + size_v) > MAX_PKT_LENGTH) {
    size_v = MAX_PKT_LENGTH - currentLength;
  }
  ...
  // update length
  writeRegister(REG_PAYLOAD_LENGTH, currentLength + size_v);
